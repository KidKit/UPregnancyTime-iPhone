...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated onMainThread:(BOOL)mainThread{
    ...
    if (mainThread) {
        [self performSelectorOnMainThread:@selector(launchExecution) withObject:nil waitUntilDone:FALSE];
    }else {
